...
#else
// MinGW GCC fallback using Windows Vectored Exception Handling (VEH) and setjmp/longjmp
#include <setjmp.h>

inline thread_local jmp_buf g_seh_jmp;
inline thread_local bool g_in_seh = false;

inline LONG CALLBACK seh_veh_handler(PEXCEPTION_POINTERS ex) {
    if (g_in_seh && (ex->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION ||
                     ex->ExceptionRecord->ExceptionCode == EXCEPTION_IN_PAGE_ERROR)) {
        g_in_seh = false;
        longjmp(g_seh_jmp, 1);
    }
...
template <class Fn> bool seh_call(Fn&& fn) noexcept {
    void* handler = AddVectoredExceptionHandler(1, seh_veh_handler);
    if (!handler) {
        // Fallback: run unprotected if handler registration failed
        fn();
        return true;
    }
    g_in_seh = true;
