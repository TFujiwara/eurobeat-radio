...
public:
    ...
    // Detach the DSP from whatever channel handle it is currently installed
    // on. Used to take our DSP fully out of the radio audio path while the
    // user is on a non-target station, so the game's audio for R1-R8 flows
    // without any interference from our callback.
