...
json config_to_json(const Config& c) {
    return json{
        ...
        {"internet_radio",
         json{
             {"enabled", c.internet_radio.enabled},
             {"stream_url", c.internet_radio.stream_url},
             {"station_name", c.internet_radio.station_name},
         }},
        {"audio",
         json{
...
void apply_patch(Config& c, const json& j) {
    ...
    if (auto it = j.find("internet_radio"); it != j.end()) {
        c.internet_radio.enabled      = pull(*it, "enabled", c.internet_radio.enabled);
        c.internet_radio.stream_url   = pull(*it, "stream_url", c.internet_radio.stream_url);
        c.internet_radio.station_name = pull(*it, "station_name", c.internet_radio.station_name);
    }
