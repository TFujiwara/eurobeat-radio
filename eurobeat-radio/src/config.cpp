...
Config load_config(const std::filesystem::path& path) {
    ...
    const auto& ir = section(root, "internet_radio");
    cfg.internet_radio.enabled = pick<bool>(ir, "enabled", cfg.internet_radio.enabled);
    cfg.internet_radio.stream_url = pick<std::string>(ir, "stream_url", cfg.internet_radio.stream_url);
    cfg.internet_radio.station_name = pick<std::string>(ir, "station_name", cfg.internet_radio.station_name);
...
void save_config(const std::filesystem::path& path, const Config& cfg) {
    ...
    e.header("internet_radio");
    e.kv("enabled", cfg.internet_radio.enabled);
    e.kv("stream_url", cfg.internet_radio.stream_url);
    e.kv("station_name", cfg.internet_radio.station_name);
