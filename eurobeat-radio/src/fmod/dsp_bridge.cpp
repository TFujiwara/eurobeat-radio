...
void DSPBridge::uninstall() noexcept {
    if (current_handle_.load(std::memory_order_relaxed)) {
        log::info("[dsp] uninstalling DSP (off target station)");
        release_current_dsp_locked();
    }
}
...
uint32_t __stdcall DSPBridge::read_callback(void* /*dsp_state*/, float* in_buf, float* out_buf,
    ...
    // In passthrough mode, preserve the input's channel layout — overriding
    // out_channels confuses FMOD's downstream mixer for the game's own radio
    // streams (R1-R8) whose channel layout differs from ours, causing it to
    // silence the channel after ~1s. Only declare our preferred layout when
    // we're actually generating audio (PCM/silence modes).
    if (m == DSPMode::passthrough) {
        const int32_t pass_ch = in_channels > 0 ? in_channels : 2;
        if (out_channels) *out_channels = pass_ch;
        const std::size_t total = static_cast<std::size_t>(length) * pass_ch;
        ...
        b->last_out_ch_.store(pass_ch, std::memory_order_relaxed);
        return 0;
    }
