...
constexpr const char* kTargetStation1 = "Horizon Opus";
constexpr const char* kTargetStation2 = "Opus";
...
GameStateProbe::Snapshot GameStateProbe::read() const noexcept {
    Snapshot out{};
    if (!singleton_slot_) {
        // Probe failed to locate the radio_state singleton at startup.
        // Assume we are on the target station so the DSP stays in pcm mode
        // and Eurobeat continues to play rather than silencing all radio.
        out.on_target_station = true;
        return out;
    }
