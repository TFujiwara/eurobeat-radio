#!/usr/bin/env python3
"""Apply custom BC7 logos to Forza Horizon 6's Anthem.zip.

Basado en el enfoque de FH6-Custom-RaDX pero usando DDS directamente.
"""
import math
import shutil
import struct
...
def apply_logos(dds_map: dict[int, tuple[Path, int, int]], game_root: Path) -> None:
    """Apply logos to Anthem.zip.

    dds_map: {station_num: (dds_path, width, height)}
    """
    anthem = game_root / "media/UI/Textures/Anthem.zip"
    backup = game_root / "media/UI/Textures/Anthem.zip.py_backup"

    if not anthem.exists():
        raise FileNotFoundError(f"Anthem.zip not found at {anthem}")
    ...
    print(f"[install] Anthem.zip written ({len(entries)} entries)")
...
if __name__ == "__main__":
    ...
    logos = {
        9: (Path("F:/WIP/EuroBeatFH6/LogoRadio/Horizon_Opus2.dds"), 247, 115),
    }
